...
/*   Updated: 2024/12/17 16:46:27 by francis          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_tokenize_redir_append(char *prompt, int i, t_token **tok)
{
	if (prompt[i] == '>' && prompt[i - 1] == '>'
		&& ft_token_has_open_quote(*tok) == false)
	{
		free((*tok)->str);
		(*tok)->str = ft_strdup(">>");
...
//
// How it works:
// - If current token has more than one character in it (ft_strlen),
//   it delimits it because it has already been checked
//   checked to be an operator token previously
...
// IF
// - previous character used as part of an operator
// - current character cannot be used with 
